...
int  CZipArchive::CloseFile(LPCTSTR lpszFilePath, bool bAfterException)
{
	...
	else
	{
		...
		{			
			...
			//		iRet = -2;
			// KTY FIX : 파일 날짜 갱신 함수인 utime이 2038년도까지만 수정이 가능하므로 에러 처리를 하지 않도록 변경함.
			// 날짜를 변경하지 않는다고 해서 문제될 사항은 없음.
			ZipPlatform::SetFileModTime(lpszFilePath, CurrentFile()->GetTime());
			if(!ZipPlatform::SetFileAttr(lpszFilePath, CurrentFile()->GetSystemAttr()))
				iRet = -2;
		}
		if (m_pCryptograph)
