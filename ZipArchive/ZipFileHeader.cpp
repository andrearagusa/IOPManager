...
bool CZipFileHeader::Read(CZipCentralDir& centralDir, bool bReadSignature)
{
	...
	CBytesWriter::ReadBytes(uFileNameSize,	    buf + 4);
	CBytesWriter::ReadBytes(uExtraFieldSize,	buf + 6);
	...
	CBytesWriter::ReadBytes(m_uVersionMadeBy,	buf + 28);
	CBytesWriter::ReadBytes(m_uVersionNeeded,	buf + 30);
...
DWORD CZipFileHeader::Write(CZipStorage *pStorage)
{
	...
	CBytesWriter::WriteBytes(dest + 4,  uFileNameSize);
	CBytesWriter::WriteBytes(dest + 6,  uExtraFieldSize);
	...
	CBytesWriter::WriteBytes(dest + 28, m_uVersionMadeBy);
	CBytesWriter::WriteBytes(dest + 30, m_uVersionNeeded);
...
bool CZipFileHeader::ReadLocal(CZipCentralDir& centralDir)
{
	...
	bool bIsDataDescr = (((WORD)*(buf + 28)) & 8) != 0;
	
	WORD uTemp; 
	CBytesWriter::ReadBytes(uTemp, buf + 28);
	...
	CBytesWriter::ReadBytes(m_uLocalFileNameSize, buf + 4); 
...
{	
	...
	CBytesWriter::WriteBytes(dest + 4,  m_uLocalFileNameSize);
	CBytesWriter::WriteBytes(dest + 6,  (WORD)uExtraFieldSize);
	...
	CBytesWriter::WriteBytes(dest + 26, m_uVersionNeeded);
	CBytesWriter::WriteBytes(dest + 28, m_uFlag);
